...
@section('content')
    ...
    <div class="row col-lg-12 col-sm-12 col-md-12 container-anexo">
        ...
        @if ($cxc->movimiento != 'Mensualidad '.$cxc->folioMov)
            <div class="col-lg-12 col-sm-12 n-contract">
                <h5 class="text-black">{{$cxc->movimiento}} {{$cxc->folioMov}}</h5>
            </div>
        @else
            <div class="col-lg-12 col-sm-12 n-contract">
                <h5 class="text-black">{{$cxc->movimiento}}</h5>
            </div>
        @endif
        <div class="gallery">
            @foreach($docsCxC as $document)
            <?php
                $srcDoc = "";
                // nombre de los files
            ...
            <form class="col-lg-12 form-upload" method="POST" action="{{ route('proc.cxc.createAnexo', ['id' => $lastID]) }}" accept-charset="UTF-8" enctype="multipart/form-data">
