...
@endsection
{{-- @php
dd($ventas->movimiento, 'Mensualidad ' .$ventas->folioMov);
@endphp --}}
@section('content')
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.0/jquery.min.js"></script>
    <div class="row col-lg-12 col-sm-12 col-md-12 container-anexo">
        <div class="title col-lg-12 col-sm-12">
            <h2 class="text-black">Documentos digitales</h2>
        </div>
        @if ($ventas->movimiento != 'Mensualidad '.$ventas->folioMov)
            <div class="col-lg-12 col-sm-12 n-contract">
                <h5 class="text-black">{{$ventas->movimiento}} {{$ventas->folioMov}}</h5>
            </div>
        @else
            <div class="col-lg-12 col-sm-12 n-contract">
                <h5 class="text-black">{{$ventas->movimiento}}</h5>
            </div>
        @endif
        <div class="gallery">
            @foreach($docsVentas as $document)
            <?php
                $srcDoc = "";
                // nombre de los files
            ...
            <form class="col-lg-12 form-upload" method="POST" action="{{ route('proc.venta.createAnexo', ['id' => $lastID]) }}" accept-charset="UTF-8" enctype="multipart/form-data">
    ...
    @if(isset($paypal))
        <div class="row" >
            ...
            <div class="col-lg-6" style="margin: auto">
                <div class="token-info bg-ligth">
                    <h4 class="title title-md mb-2 text-sm-center">PayPal Transacción</h4>
                    <table class="table table-token">
                        <tbody>
                            <tr>
                                <td class="table-head">ID Orden PayPal</td>
                                <td class="table-des">{{ $paypal->idOrden}}</td>
                            </tr>
                            <tr>
                                <td class="table-head">ID Pago PayPal</td>
                                <td class="table-des">{{ $paypal->idPago}}</td>
                            </tr>
                            <tr>
                                <td class="table-head">Fecha</td>
                                <td class="table-des">{{ $paypal->fecha}}</td>
                            </tr>
                            <tr>
                                <td class="table-head">Total Bruto</td>
                                <td class="table-des">${{ number_format($paypal->total, 2)}}</td>
                            </tr>
