...
<section class="bg-white">
    <div class="row">
        ...
        <div class="col-lg-2 col-sm-12 float-left">
            <a href="{{route("config.roles.create")}}" class="btn btn-outline btn-sm btn-success float-left">Crear Rol</a>
        </div>
    ...
    {!! Form::open(['route' => 'config.roles.filtro', 'method'=>'POST']) !!}
    {{-- <form  action="{{ route('config.usuarios.filtro')}}" method="post"> --}}
        {{-- @csrf --}}
        @php
        // dd(session()->get('status'),  session()->has('status')); 
        @endphp
    <div class="row row row-cols-1 row-cols-sm-2 row-cols-md-4">
        ...
        <div class="col-sm-6 col-lg-6">
            ...
            {!! Form::select('selectEstatus', ['Todos' => 'Todos', '1' => 'Alta', '0' => 'Baja'], session()->has('status2') ? session()->get('status2') : '1', array('id' => 'selectEstatus', "class" => '')) !!} 
        </div>

        <div class="col-lg-12 ">
            {{-- <div class="botonesM">
            <a href="{{route('config.roles.index')}}" type="reset" class="btn btn-outline btn-sm btn-light mt-3">Restablecer</a>
            ...
            <div class="botonesM">
                <a href="{{ route('config.roles.index') }}" type="reset"
                        ...
                        <ul class="menu-mega-list" id="shCol">
                            ...
                            <li>
                                <div class="ckbox ckbox-primary">
                                    ...
                                    {!!Form::label('checkDescripcion', 'Descripcion', array('class' => 'negrita'))!!}
                                </div>
                            </li>
                            <li>
                                <div class="ckbox ckbox-primary">
                                    {!!Form::checkbox('Estatus', '4', true, ['id' => 'checkEstatus'])!!}
                                    {!!Form::label('checkEstatus', 'Estatus', array('class' => 'negrita'))!!}
                                </div>
                            </li>
                            <li>
                                <div class="ckbox ckbox-primary">
                                    {!!Form::checkbox('creado', '5', false, ['id' => 'checkCreado'])!!}
                                    {!!Form::label('checkCreado', 'Creado', array('class' => 'negrita'))!!}
                                </div>
                            </li>
                            <li>
                                <div class="ckbox ckbox-primary">
                                    {!!Form::checkbox('Actualizado', '6', false, ['id' => 'checkActualizado'])!!}
                                    {!!Form::label('checkActualizado', 'Actualizado', array('class' => 'negrita'))!!}
                                </div>
                            </li>
                        </ul>
                    </div>
                    ...
                    <table  class="table table-striped table-bordered widthAll datatable" id="shTable">
                        <thead>
                            <tr>
                                ...
                                <th class="table-head">Creado</th>
                                <th class="table-head">Actualizado</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if(session()->has('rol_filtro'))
                                @foreach (session('rol_filtro') as $rol)
                                    @include('include.config.rolesItem')
                                @endforeach
                                @else
