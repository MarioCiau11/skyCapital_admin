...
<main class="ath-body">
    <section class="bg-white">
            ...
            <form action="{{ route('agrup.grupo.update', Crypt::encrypt($Grupo->idGrupo))}}" class="form-validate" method="post" id="form-edit">
                @method('PATCH')
                    @csrf
                    @include('page.agrup.Agentes.grupo.form')
                <input class="btn btn-grad" type="submit" value="Editar grupo">
                <a href="{{route('agrup.grupo.index')}}" name="accion" class="btn btn-light" type="submit">cancelar</a>
            </form>
