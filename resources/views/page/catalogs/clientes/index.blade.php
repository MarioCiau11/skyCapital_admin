
@extends('layouts.layout')

@section('content')
<section class="bg-white">
    <div class="row">
        <div class="col-lg-6">
            <div class="nk-block-text">
                <h2 class="title">Clientes</h2>
            </div>
        </div>
        <div class="col-lg-6 col-md-12 col-sm-12 contenedorBotones"> 
            <div class="item">
                <a class="btn btn-dark btn-sm btn-with-icon btn-alt btn-outline toggle-tigger" href="#">
                    <span>Grupos</span>
                    <em class="icon fas fa-caret-down"></em>
                </a>
                <ul class="drop-list drop-list-full drop-list-center toggle-class toggle-drop">
                    <li><a href="{{ route('agrup.clientes.grupo.create') }}" class=""><i class="fa fa-check-circle" aria-hidden="true"></i> Crear</a></li>
                    <li><a href="{{ route('agrup.clientes.grupo.index') }}" class=""><i class="fa fa-eye" aria-hidden="true"></i> Ver</a></li>
                </ul>
            </div>

                
            <div class="item">
                <a class="btn btn-dark btn-sm btn-with-icon btn-alt btn-outline toggle-tigger" href="#">
                    <span>Categorías</span>
                    <em class="icon fas fa-caret-down"></em>
                </a>
                <ul class="drop-list drop-list-full drop-list-center toggle-class toggle-drop">
                    <li><a href="{{route('agrup.clientes.categoria.create')}}" class=""><i class="fa fa-check-circle" aria-hidden="true"></i> Crear</a></li>
                    <li><a href="{{ route('agrup.clientes.categoria.index') }}" class=""><i class="fa fa-eye" aria-hidden="true"></i>Ver</a></li>
                </ul>
            </div>

            <div class="item">
                    <a href="{{ route('cat.clientes.create') }}" class="btn btn-outline btn-sm btn-success ">
                    Crear
                    Cliente
                </a>
            </div>
        </div>
    </div>

    <div class="row mt-2">
        <div class="col-lg-12">
            {{ Breadcrumbs::render() }}
        </div>
    </div>

    {!! Form::open(['route' => 'cat.clientes.filtro', 'method' => 'POST']) !!}
    <div class="row row row-cols-1 row-cols-sm-2 row-cols-md-4">
        <div class="col-lg-2">
            {!! Form::label('inputClave', 'Clave', ['class' => 'field-label']) !!}
            {!! Form::text('inputClave', session()->has('inputClave')? session()->get('inputClave'): null, [
            'class' => 'input-bordered',
            'id' => 'inputClave'
            ]) !!}
        </div>
        <div class="col-lg-2">
            {!! Form::label('inputName', 'Nombre', ['class' => 'field-label']) !!}
            {!! Form::text('inputName', session()->has('inputName') ? session()->get('inputName') : null, [
                'class' => 'input-bordered',
                'id' => 'inputName'
            ]) !!}
        </div>
        <div class="col-lg-2">
            {!! Form::label('inputRazon', 'Razón social', ['class' => 'field-label']) !!}
            {!! Form::text('inputRazon', session()->has('inputRazon')? session()->get('inputRazon'):null, [
                'class' => 'input-bordered',
                'id' => 'inputRazon'
            ]) !!}
        </div>
        <div class="col-lg-2">
            {!! Form::label('selectCategoria', 'Categoría', ['class' => 'field-label']) !!}
            {!! Form::select(
                'selectCategoria',
                ['Todos' => 'Todos'] + $categoria,
                session()->has('selectCategoria') ? session()->get('selectCategoria') : null,
                ['id' => 'selectCategoria'],
            ) !!}
        </div>
        <div class="col-lg-2">
            {!! Form::label('selectGrupo', 'Grupo', ['class' => 'field-label']) !!}
            {!! Form::select(
                'selectGrupo',
                ['Todos' => 'Todos'] + $grupo,
                session()->has('selectGrupo') ? session()->get('selectGrupo') : null,
                ['id' => 'selectGrupo'],
            ) !!}
        </div>
        <div class="col-lg-2">
            {!! Form::label('selectEstatus', 'Estatus', ['class' => 'field-label']) !!}
            {!! Form::select(
                'selectEstatus',
                ['1' => 'Alta', '0' => 'Baja', 'Todos' => 'Todos'],
                session()->has('selectEstatus') ? session()->get('selectEstatus') : 'Todos',
                ['id' => 'selectEstatus', 'class' => ''],
            ) !!}
        </div>
        <div class="col-lg-12 ">
            <div class="botonesM">
            <a href="{{ route('cat.clientes.index') }}" type="reset"
                class="btn btn-outline btn-sm btn-light  btn-auto-2 float-left mt-3">Restablecer</a>
            {!! Form::submit('Búsqueda', ['class' => 'btn btn-outline btn-sm btn-primary mt-3', 'name' => 'action']) !!}
            {!! Form::submit('Exportar excel', ['class' => 'btn btn-outline btn-sm btn-success mt-3', 'name' => 'action']) !!}
            {!! Form::close() !!}
            </div>

            {{-- columnas --}}
            <ul class="menu menu2 mx-auto btn btn-outline btn-sm btn-light btn-auto float-right mt-2" >
                <li class="menu-item has-sub">
                    <a class="menu-link nav-link menu-toggle typografy" href="#">Columnas</a>
                    <div class="menu-sub menu-drop menu-mega ">
                        <div class="menu-mega-innr">
                            <ul class="menu-mega-list" id="shCol" style="z-index:1000;">
                                <li class="menu-item">
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Opciones', '0', true, ['id' => 'checkOpciones']) !!}
                                        {!! Form::label('checkOpciones', 'Opciones', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Clave', '1', true, ['id' => 'checkclave']) !!}
                                        {!! Form::label('checkclave', 'Clave', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Razón social', '2', true, ['id' => 'checkRazon']) !!}
                                        {!! Form::label('checkRazon', 'Razón social ', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('RFC', '3', false, ['id' => 'checkRFC']) !!}
                                        {!! Form::label('checkRFC', 'RFC', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('CURP', '4', false, ['id' => 'checkCURP']) !!}
                                        {!! Form::label('checkCURP', 'CURP', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Observaciones', '5', false, ['id' => 'checkObservaciones']) !!}
                                        {!! Form::label('checkObservaciones', 'Observaciones', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Ocupación', '6', false, ['id' => 'checkOcupacion']) !!}
                                        {!! Form::label('checkOcupacion', 'Ocupación', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Grupo', '7', true, ['id' => 'checkGrupo']) !!}
                                        {!! Form::label('checkGrupo', 'Grupo', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Categoria', '8', true, ['id' => 'checkCategoria']) !!}
                                        {!! Form::label('checkCategoria', 'Categoría', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Condición de Pago', '9', false, ['id' => 'checkCondicion']) !!}
                                        {!! Form::label('checkCondicion', 'Condición de Pago', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Usuario', '10', false, ['id' => 'checkUsuario']) !!}
                                        {!! Form::label('checkUsuario', 'Usuario', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Estatus', '11', true, ['id' => 'checkEstatus']) !!}
                                        {!! Form::label('checkEstatus', 'Estatus', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Creado', '12', false, ['id' => 'checkCreado']) !!}
                                        {!! Form::label('checkCreado', 'Creado', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Actualizado', '13', false, ['id' => 'checkActualizado']) !!}
                                        {!! Form::label('checkActualizado', 'Actualizado', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                                <li>
                                    <div class="ckbox ckbox-primary">
                                        {!! Form::checkbox('Baja', '14', false, ['id' => 'checkFechab']) !!}
                                        {!! Form::label('checkFechab', 'Baja', ['class' => 'negrita']) !!}
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </li>
            </ul>
            {{-- fin columnas --}}
        </div>
    </div>
    <div class="row mt-2">
        <div class="col-lg-12">
            <div class=" table-responsive">
                <div class="datatable-container">
                    <table class="table table-striped table-bordered widthAll datatable" id="shTable">
                        <thead>
                            <tr>
                                <th class="table-head">Opciones</th>
                                <th class="table-head">Clave</th>
                                <th class="table-head">Nombre/Razón Social</th>
                                <th class="table-head">RFC</th>
                                <th class="table-head">CURP</th>
                                <th class="table-head">Observaciones/Comentarios</th>
                                <th class="table-head">Ocupación</th>
                                <th class="table-head">Grupo</th>
                                <th class="table-head">Categoría</th>
                                <th class="table-head">Condición de Pago</th>
                                <th class="table-head">Usuario que lo Registró</th>
                                <th class="table-head">Estatus</th>
                                <th class="table-head">Fecha de Alta</th>
                                <th class="table-head">Última Actualización</th>
                                <th class="table-head">Fecha de Baja</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if (session()->has('clientes_filtro'))
                                @foreach (session('clientes_filtro') as $cliente)
                                    @include('include.catalogs.clienteItem')
                                @endforeach
                            @else
                            @foreach ($Clientes as $cliente)
                                @include('include.catalogs.clienteItem')
                            @endforeach
                            @endif
                            
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</section>
@include('include.mensaje')
@endsection

@section('extra-js')
    <script src="{{asset('js\catalogos\clientes.js')}}"></script>
@endsection