...
class ClientesCategoriaController extends Controller
{
    private $categoriaBD;
    public $message;
    public $status;
    ...
    {
        $categoria = AGRUP_CATEGORIA::where('catalogo','Clientes')->get();
        return view('page.agrup.Clientes.categoria.index',[
            'Categoria' => $categoria
        ]);
    }
    ...
    {
        return view('page.agrup.Clientes.categoria.create',[
    ...
    public function store(Request $request)
    {
        ...
        $categoria->catalogo = 'Clientes';
        try {
            $isCreated = $categoria->save();
            if ($isCreated) {
                $this->message = 'Se ha creado la categoría correctamente';
                $this->status = true;
            }
            else{
            ...
            return redirect()
            ->route('agrup.clientes.categoria.index')
            ->with('message',$this->message)
            ->with('status',$this->status);
        } catch (\Exception $e) {
            $this->message = 'Error en crear la categoría '.$e;
            $this->status = false;
            return redirect()
            ->route('agrup.clientes.categoria.index')
            ->with('message',$this->message)
    ...
    public function edit(string $id)
    {
        ...
        return view('page.agrup.Clientes.categoria.edit',[
            'Categoria' => $categoria
        ]);
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            return redirect()->route('agrup.clientes.categoria.index')
                ->with('message', $message)
                ->with('status', $status);
        } catch (\Throwable $th) {
            return redirect()->route('agrup.clientes.categoria.index')
