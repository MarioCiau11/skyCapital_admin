...
class AgentesGrupoController extends Controller
{
    ...
    {
        $grupo = AGRUP_GRUPO::where('catalogo','Agentes de venta')->get();
        return view('page.agrup.Agentes.grupo.index',[
            'Grupo' => $grupo
        ]);
    }
    ...
    {
        return view('page.agrup.Agentes.grupo.create',[
    ...
    public function store(Request $request)
    {
        ...
        $grupo->catalogo = 'Agentes de venta';
        try {
            ...
            return redirect()
            ->route('agrup.grupo.index')
            ->with('message',$this->mensaje)
            ->with('status',$this->status);
        } catch (\Exception $e) {
            $this->mensaje = 'Error en crear el '.$e;
            $this->status = false;
            return redirect()
            ->route('agrup.grupo.index')
            ->with('message',$this->mensaje)
    ...
    public function edit(string $id)
    {
        ...
        return view('page.agrup.Agentes.grupo.edit',[
            'Grupo' => $grupo
        ]);
    }
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            try {
                ...
                if ($isUpdate) {
                    $message = 'El grupo '.$data['inputName'].' ha sido actualizada correctamente';
                    $status = true;
                 }
                 else {
    ...
    public function destroy(string $id)
    {
        try {
            ...
            return redirect()->route('agrup.grupo.index')
                ->with('message', $message)
                ->with('status', $status);
        } catch (\Throwable $th) {
            return redirect()->route('agrup.grupo.index')
